...
describe('FilterBar', () => {
  ...
  it('counts deviations and exposes clear and scenario-reset behavior', async () => {
    ...
    await user.click(screen.getByRole('checkbox', { name: 'Yalnızca ücretsiz katmanlar' }))
    await user.click(screen.getByRole('checkbox', { name: 'Eski verileri dahil et' }))
    ...
    expect(screen.getByRole('checkbox', { name: 'Yalnızca ücretsiz katmanlar' })).not.toBeChecked()
    expect(screen.getByRole('checkbox', { name: 'Eski verileri dahil et' })).not.toBeChecked()
