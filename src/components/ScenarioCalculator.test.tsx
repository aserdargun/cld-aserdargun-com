...
describe('ScenarioCalculator', () => {
  it('groups active preset fields and exposes the mobile scenario control', async () => {
    ...
    expect(screen.queryByRole('spinbutton', { name: 'Aylık GPU kullanımı' })).not.toBeInTheDocument()
    await user.click(within(advanced!).getByText('Gelişmiş kullanım ayarları'))
    expect(screen.getByRole('spinbutton', { name: 'Aylık GPU kullanımı' })).toBeInTheDocument()
  ...
  it('offers all six presets and selects the AI / GPU scenario', async () => {
    ...
    expect(scenarioSelect).toHaveTextContent('Yüksek trafikli uygulama')
